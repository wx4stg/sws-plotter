#!/usr/bin/env python3
# Plotting data from SWS dat files (TASC CM-100, AIO2, etc.) generated by WView
# Created 4 June 2022 by Sam Gardner <stgardner4@tamu.edu>

from os import path, listdir
from pathlib import Path
import pandas as pd
from datetime import datetime as dt, timedelta
from matplotlib import pyplot as plt
from matplotlib.gridspec import GridSpec
from matplotlib import dates as pltdates
from matplotlib import image as mplimage
from metpy import calc as mpcalc
from metpy.units import units

if __name__ == "__main__":
    # Get directory this script runs from
    basePath = path.realpath(path.dirname(__file__))
    # Get input subdir
    inputDir = path.join(basePath, "input")
    # Create that subdirectory if it doesn't already exist
    Path(inputDir).mkdir(parents=True, exist_ok=True)
    # Loop through all files in input directory
    for datFile in sorted(listdir(inputDir)):
        if ".dat" in datFile:
            
            datFilePath = path.join(inputDir, datFile)
            swsTable = pd.read_csv(datFilePath)
            startYear = swsTable["Year"][0]
            dtsList = list()
            for idx, row in swsTable.iterrows():
                dtsList.append(dt(row["Year"], 1, 1)+timedelta(days=row["Day"])+timedelta(hours=int(str(row["HrMn"])[:2]))+timedelta(minutes=int(str(row["HrMn"])[2:])))
            swsTable["datetimes"] = dtsList
            swsTable = swsTable.set_index(["datetimes"])

            # Convert celcius temps to fahrenheit
            swsTable["TempF"] = swsTable["Temp"] * 1.8 + 32
            # Get dew point from RH
            swsTable["dewPoint"] = [mpcalc.dewpoint_from_relative_humidity(swsTable["TempF"][i] * units.degF, swsTable["Humid"][i]/100).to("degF").magnitude for i in range(len(swsTable["Humid"]))]

            # calculate u and v wind components for barbs
            windTimes = list()
            u = list()
            v = list()
            for i in range(0, len(swsTable["WSpd"])):
                time = swsTable.index[i]
                windTimes.append(time)
                spd = units.Quantity(swsTable["WSpd"][i], "m/s")
                dir = units.Quantity(swsTable["WDir"][i], "degrees")
                uwind, vwind = mpcalc.wind_components(spd, dir)
                u.append(uwind.to(units("knots")).magnitude)
                v.append(vwind.to(units("knots")).magnitude)

            fig = plt.figure()
            px = 1/plt.rcParams["figure.dpi"]
            fig.set_size_inches(1920*px, 1080*px)
            gs = GridSpec(4, 3, figure=fig)
            gs.update(hspace = 50*px)
            ax1 = fig.add_subplot(gs[0, :])
            ax1.plot(swsTable["TempF"], "red", label="Temperature")
            ax1.scatter(swsTable.index, swsTable["TempF"], 1, "red")
            ax1.plot(swsTable["dewPoint"], "green", label="Dew Point")
            ax1.scatter(swsTable.index, swsTable["dewPoint"], 1, "green")
            ax1.legend(loc="upper left")
            ax1.set_title("Temperature/Dew Point")
            ax1.xaxis.set_major_formatter(pltdates.DateFormatter("%m/%d %H:%M"))
            ax1.set_ylabel("°F")
            ax2 = fig.add_subplot(gs[1, :])
            ax2.barbs(windTimes, 0, u, v, pivot="middle", label="Winds")
            ax2.legend(loc="upper left")
            ax2.set_title("Winds")
            ax2.tick_params(left=False, labelleft=False)
            ax2.xaxis.set_major_formatter(pltdates.DateFormatter("%m/%d %H:%M"))
            ax4 = fig.add_subplot(gs[2, :])
            ax4.plot(swsTable["Press"], "blue", label="Barometric Pressure")
            ax4.scatter(swsTable.index, swsTable["Press"], 1, "blue")
            ax4.set_title("Barometer")
            ax4.set_ylabel("hPa")
            ax4.legend(loc="upper left")
            ax4.xaxis.set_major_formatter(pltdates.DateFormatter("%m/%d %H:%M"))
            ax5 = fig.add_subplot(gs[3, :])
            ax6 = ax5.twinx()
            battplot = ax6.plot(swsTable["User2"], "green", label="Battery Voltage")
            ax6.scatter(swsTable.index, swsTable["User2"], 1, "green")
            ax6.xaxis.set_major_formatter(pltdates.DateFormatter("%m/%d %H:%M"))
            ax6.set_ylabel("Volts")
            try:
                swsTable["Rad"] = swsTable["Rad"].astype(float)
                srplot = ax5.plot(swsTable["Rad"], "yellow", label="Solar Radiation")
                ax5.scatter(swsTable.index, swsTable["Rad"], 1, "yellow")
                ax5.set_ylabel("W m^-2")
                fourthPlotLines = srplot+battplot
            except:
                fourthPlotLines = battplot
            ax6.legend(fourthPlotLines, [line.get_label() for line in fourthPlotLines], loc="upper left")
            ax6.set_title("Solar and Battery")
            fig.subplots_adjust(bottom = 0.21)
            fig.subplots_adjust(top = 0.95)
            fig.subplots_adjust(right = 0.95)
            fig.subplots_adjust(left = 0.05)
            fig.set_size_inches(1920*px, 1080*px)
            tax = fig.add_axes([ax6.get_position().x0+(ax6.get_position().width/2)-(ax6.get_position().width/6),0.1,(ax6.get_position().width/3),.05])
            tax.text(0.5, 0.6, "TAMU TRACER AIO2 Weather Station\nValid "+swsTable.index[0].strftime("%b %d %H:%M")+" through "+swsTable.index[-1].strftime("%b %d %H:%M"), horizontalalignment="center", verticalalignment="center", fontsize=16)
            plt.setp(tax.spines.values(), visible=False)
            tax.tick_params(left=False, labelleft=False)
            tax.tick_params(bottom=False, labelbottom=False)
            lax = fig.add_axes([.75,0.01,0.2,0.2])
            atmoLogo = mplimage.imread("assets/atmoLogo.png")
            lax.imshow(atmoLogo)
            lax.axis("off")
            prodDir = path.join(basePath, "output")
            Path(prodDir).mkdir(parents=True, exist_ok=True)
            fig.savefig(path.join(prodDir, datFile.replace(".dat", ".png")))
    